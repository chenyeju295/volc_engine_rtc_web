...
class RtcEngineManager {
  ...
  Future<void> _initializeRtcEngine() async {
    ...
    try {
      ...
      // 启用音频属性报告
      js_util.callMethod(engine, 'enableAudioPropertiesReport', [js_util.jsify({'interval': 1000})]);
      debugPrint('【RTC引擎】已启用音频属性报告，间隔: 1000ms');
      
      // 尝试添加AI降噪扩展（可选，如果失败不会影响主要功能）
      try {
        debugPrint('【RTC引擎】尝试加载AI降噪扩展...');
        final rtcAiAnsExtensionClass = js_util.getProperty(js_util.globalThis, 'RTCAIAnsExtension');
        if (rtcAiAnsExtensionClass != null) {
          final aiAnsExtension = js_util.callConstructor(rtcAiAnsExtensionClass, []);
          await js_util.promiseToFuture(js_util.callMethod(engine, 'registerExtension', [aiAnsExtension]));
  ...
  void registerEventHandlers({
    ...
    try {
      ...
      _registerEvent(events, 'onUserJoined', onUserJoined ?? (dynamic userData) {
        debugPrint('【事件系统】用户加入: ${userData is Map ? userData['userId'] : userData}');
      });
      
      _registerEvent(events, 'onUserLeave', onUserLeave ?? (dynamic userData) {
        debugPrint('【事件系统】用户离开: ${userData is Map ? userData['userId'] : userData}');
      });
      
      _registerEvent(events, 'onRoomBinaryMessageReceived', onBinaryMessage ?? (dynamic messageData) {
      ...
      _registerEvent(events, 'onAutoplayFailed', onAutoPlayFail ?? (dynamic errorData) {
        debugPrint('【事件系统】自动播放失败: $errorData');
      });
      
      _registerEvent(events, 'onUserStartAudioCapture', onUserStartAudioCapture ?? (dynamic userData) {
        final userId = userData is Map ? userData['userId'] : userData;
        debugPrint('【事件系统】用户开始音频采集: $userId');
      });
      
      _registerEvent(events, 'onUserStopAudioCapture', onUserStopAudioCapture ?? (dynamic userData) {
        final userId = userData is Map ? userData['userId'] : userData;
  ...
  Future<bool> joinRoom({required String roomId, required String userId, required String token}) async {
    if (engine == null) {
    ...
    try {
      debugPrint('【加入房间】正在加入RTC房间: ${roomId.toLowerCase()}');
      debugPrint('【加入房间】token: ${token != null ? '已设置' : '未设置'}');

      // 准备用户信息
      final validatedUserId = _validateUserId(userId).toLowerCase();
      ...
      debugPrint('【加入房间】用户ID: $validatedUserId, 附加信息: $extraInfo');
      ...
      // 准备用户对象
      final userObject = js_util.jsify({
      ...
      await js_util.promiseToFuture(js_util.callMethod(
        engine, 
        'joinRoom', 
        [token, roomId.toLowerCase(), userObject, roomOptions]
      ));
  ...
  Future<Map<String, dynamic>> checkPermissionAndGetDevices() async {
    ...
    try {
      ...
      // 请求权限
      final permissionResult = await js_util.promiseToFuture<Map<dynamic, dynamic>>(
      ...
      if (hasAudioPermission) {
        // 获取音频输入设备
        final inputsResult = await js_util.promiseToFuture(
          js_util.callMethod(vertcObject, 'enumerateAudioCaptureDevices', [])
        );
        
        // 获取音频输出设备
        final outputsResult = await js_util.promiseToFuture(
          js_util.callMethod(vertcObject, 'enumerateAudioPlaybackDevices', [])
        );
  ...
  Future<bool> resumeAudioPlayback() async {
    ...
    try {
      ...
      if (AudioContext != null) {
        ...
        js_util.callMethod(oscillator, 'connect', [js_util.getProperty(audioContext, 'destination')]);
        js_util.callMethod(oscillator, 'start', [0]);
        js_util.callMethod(oscillator, 'stop', [0.1]);
      }
      
      // 尝试播放一个静音音频
      js_util.setProperty(audioElement, 'src', 'data:audio/wav;base64,UklGRiQAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAZGF0YQAAAAA=');
