...
class RtcMessageHandler {
  ...
  void handleBinaryMessage(String userId, dynamic message) {
    ...
    try {
      ...
      // 尝试解析TLV格式
      final Map<String, dynamic>? tlvData = RtcMessageUtils.parseTlvMessage(bytes);
      if (tlvData != null) {
        debugPrint('RtcMessageHandler: 成功解析TLV格式消息');
  ...
  void _processTextOrJson(String userId, String text) {
    ...
    // 尝试解析为JSON
    try {
      final Map<String, dynamic>? jsonData = RtcMessageUtils.safeParseJson(text);
      if (jsonData != null) {
    ...
    try {
      ...
      if (data.containsKey('type')) {
        ...
        if (type == RtcMessageUtils.TYPE_SUBTITLE || type == 'subv') {
          _processSubtitleMessage(userId, data);
        }
  ...
  void _processFunctionCallMessage(String userId, Map<String, dynamic> data) {
    try {
      ...
      _messageHistoryController.add({
        'type': RtcMessageUtils.TYPE_FUNCTION_CALL,
        ...functionCallData
      });
  ...
  Future<bool> sendSubtitleMessage(String text, {bool isFinal = true}) async {
    try {
      final data = RtcMessageUtils.createSubtitleMessage(text, isFinal: isFinal);
  ...
  /// 发送函数调用结果
  Future<bool> sendFunctionCallResult(String name, Map<String, dynamic> result) async {
    try {
