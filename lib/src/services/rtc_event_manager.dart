...
class RtcEventManager {
  ...
  void _registerEventHandlers() {
    ...
    try {
      ...
      _registerEvent('onPlayerEvent', (event) {
        ...
        try {
          ...
          final rawEventType = rawEvent != null ? js_util.getProperty(rawEvent, 'type') : '';
    ...
    // 网络质量事件
    _registerEvent('onNetworkQuality', (event) {
      try {
        // 处理不同的参数格式
        int uplinkQuality = 0;
        int downlinkQuality = 0;
        
        // 情况1: event是一个包含两个参数的数组
        if (event is List && event.length >= 2) {
          uplinkQuality = _safeParseInt(event[0]);
          downlinkQuality = _safeParseInt(event[1]);
        } 
        // 情况2: event是一个对象，包含uplink和downlink属性
        else if (event != null && js_util.hasProperty(event, 'uplink') && js_util.hasProperty(event, 'downlink')) {
          uplinkQuality = _safeParseInt(js_util.getProperty(event, 'uplink'));
          downlinkQuality = _safeParseInt(js_util.getProperty(event, 'downlink'));
        }
        // 情况3: 直接传递了两个参数，此时event是第一个参数(uplink)
        else {
          uplinkQuality = _safeParseInt(event);
          // 在这种情况下，第二个参数无法通过event获取，但我们已经设计了容错逻辑
        }
        ...
        debugPrint('网络质量 - 上行: $uplinkQuality, 下行: $downlinkQuality, 平均: $quality');
