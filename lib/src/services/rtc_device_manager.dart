...
import '../services/rtc_engine_manager.dart';
...
class AudioResult {
  ...
  factory AudioResult.success([Map<String, dynamic>? trackSettings]) {
    return AudioResult(success: true, trackSettings: trackSettings);
  }
}
...
class RtcDeviceManager {
    ...
    try {
      if (engine == null) {
        ...
        return AudioResult.failure(
            '引擎未设置', AudioErrorCode.ENGINE_NOT_INITIALIZED);
      }
      ...
      try {
        ...
        final resultPromise =
            js_util.callMethod(engine, 'startAudioCapture', []);
  ...
  Future<AudioResult> stopAudioCapture() async {
    try {
      ...
      try {
        ...
        final resultPromise =
            js_util.callMethod(engine, 'stopAudioCapture', []);
  ...
  Future<bool> resumeAudioPlayback() async {
    try {
      ...
      try {
        ...
        await js_util
            .promiseToFuture(js_util.callMethod(audioElement, 'play', []));
  ...
  Future<AudioResult> startAudioPlaybackDeviceTest(
    ...
    try {
      ...
      try {
        // 调用SDK方法
        await WebUtils.callJsAsync(engine, 'startAudioPlaybackDeviceTest',
  ...
  /// @return 测试结果 AudioResult
  Future<AudioResult> startAudioDeviceRecordTest(int indicationInterval,
  ...
  /// 设备变更流
  Stream<List<Map<String, dynamic>>> get deviceChangeStream =>
